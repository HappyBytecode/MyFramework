 ...
 */
package com.lirui.lib_common.base.adapter;
...
import com.lirui.lib_common.base.adapter.animation.AlphaInAnimation;
import com.lirui.lib_common.base.adapter.animation.BaseAnimation;
import com.lirui.lib_common.base.adapter.animation.ScaleInAnimation;
import com.lirui.lib_common.base.adapter.animation.SlideInBottomAnimation;
import com.lirui.lib_common.base.adapter.animation.SlideInLeftAnimation;
import com.lirui.lib_common.base.adapter.animation.SlideInRightAnimation;
import com.lirui.lib_common.base.adapter.loadmore.LoadMoreView;
import com.lirui.lib_common.base.adapter.loadmore.SimpleLoadMoreView;
...
public abstract class BaseQuickAdapter<T, K extends BaseViewHolder> extends RecyclerView.Adapter<K> {
    ...
    public void addData(@NonNull T data) {
        mData.add(data);
        notifyItemInserted(getDataCount() + getHeaderLayoutCount());
        compatibilityDataSizeChanged(1);
    }
    ...
    public void remove(@IntRange(from = 0) int position) {
        ...
        notifyItemRangeChanged(internalPosition, getDataCount() - internalPosition);
    }
    ...
    public void addData(@NonNull Collection<? extends T> newData) {
        ...
        notifyItemRangeInserted(getDataCount() - newData.size() + getHeaderLayoutCount(), newData.size());
        compatibilityDataSizeChanged(newData.size());
    }
    ...
    private void compatibilityDataSizeChanged(int size) {
        final int dataSize = mData == null ? 0 : getDataCount();
        if (dataSize == size) {
            notifyDataSetChanged();
        }
    }
   ...
   /* @Nullable
    ...
    public int getItemCount() {
        int count;
        if (getEmptyViewCount() == 1) {//显示空布局
            count = 1;
            if (mHeadAndEmptyEnable && getHeaderLayoutCount() != 0) {//空布局显示Head，并且head数量不为0
                count++;
            }
            if (mFootAndEmptyEnable && getFooterLayoutCount() != 0) {//空布局显示Foot，并且Foot数量不为0
                count++;
            }
        } else {
            count = getHeaderLayoutCount() + getDataCount() + getFooterLayoutCount() + getLoadMoreViewCount();
        }
        return count;
    }
    ...
    public void onBindViewHolder(K holder, int position) {
        ...
        switch (viewType) {
            case 0:
                //convert(position, holder, getItem(position - getHeaderLayoutCount()));
                convert(position - getHeaderLayoutCount(), holder);
                break;
            case LOADING_VIEW:
     ...
     */
    protected abstract void convert(int position, K helper);
