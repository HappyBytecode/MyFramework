 ...
 * 空/重试/加载中视图View
 *
 * @author Nukc https://github.com/nukc
 *         modify by lirui 2017/08/15
 * @Describe StateView仅起展位作用
 */
public class StateView extends View {
    ...
    public static StateView inject(@NonNull ViewGroup parent, boolean hasActionBar) {
        ...
        if (parent instanceof LinearLayout ||
            ...
            //parent 为 Fragment跟布局时，viewParent==null
            if (viewParent == null) {
    ...
    public StateView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (mEmptyResource == 0) {
            mEmptyResource = R.layout.progress_layout_empty_view;
        }
        if (mRetryResource == 0) {
            mRetryResource = R.layout.progress_layout_error_view;
        }
        if (mLoadingResource == 0) {
            mLoadingResource = R.layout.progress_layout_loading_view;
        }
    ...
    public View showRetry() {
        if (mRetryView == null) {
            ...
            mRetryView.findViewById(R.id.button_retry).setOnClickListener(new OnClickListener() {
                @Override
