package com.lirui.lib_common.view.IntensifyImage;
...
class IntensifyImageDelegate {
    ...
    private void initScaleType(Rect drawingRect) {
        ...
        //-1，缩放值为恰好显示
        if (mMinimumScale == -1) {
            mMinimumScale = (mIsVertical ? (1f * drawingRect.height() / mImage.mImageHeight)
                    : (1f * drawingRect.width() / mImage.mImageWidth));
        }
        if (mMinimumScale > mMaximumScale) {
            mMaximumScale = mMinimumScale * 2;
        }
        switch (mScaleType) {
            case NONE:
        ...
        LogUtils.d(TAG, "DrawingRect=" + drawingRect + "/ImageArea=" + imageArea);
    ...
    public void zoomScale(Rect drawingRect, float scale, float focusX, float focusY) {
        ...
        LogUtils.d(TAG, "Start=" + mStartRect + "/End=" + mEndRect);
        mZoomAnimator.start();
    }
